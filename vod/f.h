 ...
 (function() {
    ...
    deconcept.SWFObject.prototype = {
        ...
        getSWFHTML: function() {
            ...
            if (navigator.plugins && navigator.mimeTypes && navigator.mimeTypes.length) {
                ...
                '" height="' + this.getAttribute("height") + '" codebase="http://fpdownload.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=8,0,0,0">';
    ...
    deconcept.SWFObjectUtil.getPlayerVersion = function(_23, _24) {
        ...
        if (navigator.plugins && navigator.mimeTypes.length) {
            ...
            if (x && x.description) _25 = new deconcept.PlayerVersion(x.description.replace(/([a-z]|[A-Z]|\s)+/, 
            "").replace(/(\s+r|\s+b[0-9]+)/, ".").split("."))
        } else {
            try {
                var axo = new ActiveXObject("ShockwaveFlash.ShockwaveFlash");
                for (var i = 15; i > 6; i--) try {
            ...
            if (!_23 || (_23.minor != 0 || _23.rev != 0) && _25.major == _23.major || _25.major != 6 || _24) try {
    ...
    if (Array.prototype.push == null) Array.prototype.push = function(_2f) {
...
var KankanObj = XL_CLOUD_FX_INSTANCE.Class.extend({
    ...
    trace: function(str, pos) {
        if (pos == "onProgress") return false;
    ...
    getParameter: function(name) {
        ...
        if (null != matcher) return decodeURIComponent(matcher[1]);
        else return null
    },
    ...
    pageRefresh: function() {
        ...
        var arr = url.split("#");
        if (window.location.href.indexOf("?") > 0) window.location = arr[0] + "&rd=" + this.time();
        else window.location = arr[0] + "?rd=" + this.time();
        return true
    },
    $C: function(tagName) {
...
var KankanEventObj = KankanObj.extend({
    ...
    addEventListener: function(oListener, sEvent, fpNotify, tDelay) {
        if (!this[sEvent]) this[sEvent] = [];
        if (! (this[sEvent] instanceof Array)) return false;
        for (var i = 0; i < this[sEvent].length; i++) if (this[sEvent][i].o == oListener && this[sEvent][i].f == fpNotify) return true;
        this[sEvent].push({
            o: oListener,
            f: fpNotify,
            t: tDelay
    ...
    removeEventListener: function(oListener, sEvent, fpNotify) {
        if (!this[sEvent] || !(this[sEvent] instanceof Array)) return false;
        for (var i = 0; i < this[sEvent].length; i++) if (this[sEvent][i].o == oListener && (this[sEvent][i].f == fpNotify || !fpNotify)) {
            this[sEvent].splice(i, 1);
            if (0 == this[sEvent].length) delete this[sEvent];
            return true
        }
        return false
    },
    dispatchEvent: function(sEvent) {
        ...
        for (var i = 0; i < listener.length; i++) if (typeof listener[i].t == "number") listener[i].f.delayApply(listener[i].t, listener[i].o, args);
        else listener[i].f.apply(listener[i].o, args);
        return true
    }
});
var KKPlayerError = KankanEventObj.extend({
    init: function() {
        ...
        this.dErrorMap = {
            ...
            4183: "高清播放组件读取数据失败，请刷新页面重试或更换其他影片观看",
            4503: "高清播放组件读取数据失败，请刷新页面重试或更换其他影片观看",
            4504: "高清播放组件读取数据失败，请刷新页面重试或更换其他影片观看",
            4531: "网络问题，请检查网络是否连接正常，或刷新页面重试",
            4544: "无法播放此影片，请刷新页面重试或更换其他影片观看",
            4557: "高清播放组件异常，请刷新页面重试或更换其他影片观看"
        };
        this.debugBuffer = [];
    ...
    dump: function(code, isHide, envDetail) {
        ...
        if (code == "000" || code == "001" || code == "002" || code == "003" || code == "004" || code == "006" || code == "007" || code == "009") this.stat.delayCall(0, this, code, false);
        else this.stat.delayCall(0, this, code, true, envDetail);
        if (isHide) return false;
        if (typeof this.errorMap[code] != "undefined") return true;
        return false
    },
    showError: function(code, str) {
        var instance = G_PLAYER_INSTANCE;
        var errArr = str.split("|");
        if (code != 200 && typeof instance != "undefined" && instance && typeof instance.showError != "undefined") instance.showError(escape(errArr[0]), escape(errArr[1]));
        else this.showOnPage(errArr[0], code)
    },
    stat: function(code, detail, envDetail) {
        ...
        if (detail) {
            ...
            var url = this.eStatUrl + "&u1=" + code + "&u2=" + encodeURIComponent(this.getLoc()) + "&u3=" + errorInfo.sid + "&u4=" + errorInfo.lastEvent + "&u5=" + errorInfo.ctype + "_" + errorInfo.ver + "&u6=" + errorInfo.uid + "&u7=" + errorInfo.lastError + "&u8=" + this.count + 
            "&u9=" + envDetail + "&rd=" + this.time()
        } else var url = this.eStatUrl + "&u1=" + code + "&rd=" + this.time();
        this.mStatSend(url)
    },
    playCheck: function(code) {
        if (!this.errorDetectDelay) {
            ...
            this.playCheckDelay.delayCall(6E4, G_PLAYER_ERROR, code)
        }
    },
    playCheckDelay: function(code) {
        var instance = G_PLAYER_INSTANCE;
        if (!this.errorHasPlay) this.dump("T" + code, true, instance.getEventQueue() + "_" + instance.getBufferBugInfo() + "_" + instance.getPlayPosition());
        this.errorHasPlay = false;
        this.errorDetectDelay = 
        false
    }
});
var KKPlayerInit = KankanEventObj.extend({
    init: function() {
        ...
        this.playerPath ="http://vod.lixian.xunlei.com/media/vodPlayer_2.8.swf?v=2.825";
        this.dapctrl = null;
    ...
    printObjectManually: function(container, random, width, height) {
        if (this.isIE) {
            ...
            var str = "";
            str += '<embed type="application/x-shockwave-flash" src="' + this.playerPath + '" width="' + width + '" height="' + height + '" id="PlayerCtrl" name="PlayerCtrl" bgcolor="#000000" quality="high" allowscriptaccess="always" allowfullscreen="true" wmode="window" flashvars="movieType=null" pluginspage="http://www.macromedia.com/go/getflashplayer"/>';
            this.$(container).innerHTML = str
        }
    },
    checkPlayer: function() {
        ...
        if (typeof player.flv_setPlayeUrl == "undefined") this.checkPlayer.delayCall(50, this);
        else {
            ...
            try {
                if (!G_PLAYER_INIT.dapctrlBackup) G_PLAYER_INSTANCE.dapctrl.attachEvent("State", 
                G_PLAYER_INSTANCE.dapCtrlHandler);
                else G_PLAYER_INSTANCE.dapctrl.onState = G_PLAYER_INSTANCE.dapCtrlHandler
            } catch(e) {}
        }
    }
});
var KKPlayerInstance = KankanEventObj.extend({
    ...
    flashopen: function(url, isServer, urlObj, autoplay, param, ischange) {
        ...
        if (typeof this.playTimes[url] == "undefined") this.playTimes[url] = 1;
        else this.playTimes[url]++;
        ...
        if (isServer) {
            paramobj[0].totalByte = param.totalByte;
            paramobj[0].totalTime = param.totalTime;
            paramobj[0].totleTime = param.totalTime;
            paramobj[0].sliceTime = 12E3;
            paramobj[0].sliceType = 
            param.sliceType;
            paramobj[0].start = 0;
            ...
            paramobj[0].format = param.format
        }
        if (urlObj) {
            paramobj[0].jsonObj = urlObj;
            if (autoplay) paramobj[0].autoplay = autoplay;
            else paramobj[0].autoplay = 2
        }
        if (isServer) {
            this.isServer = true;
        ...
        if (param.totalTime < 10) this.bufferStart = param.totalTime;
        this.pobject.setBufferTime(this.bufferStart);
        if (ischange == 1) this.pobject.flv_setChangeUrl(paramobj);
        else this.pobject.flv_setPlayeUrl(paramobj);
    ...
    showError: function(error, suggest) {
        ...
        if (suggest == "") var suggest = "如果还无法观看，请查看<font color='#097BB3'><u><a href='javascript:G_PLAYER_INSTANCE.windowOpen(\"http://www.xunlei.com/help/\")' target='_self'>常见问题</a></u></font> 或 <font color='#097BB3'><u><a href='javascript:G_PLAYER_INSTANCE.windowOpen(\"http://www.xunlei.com/help/feedback.html" + G_PLAYER_ERROR.feedbackParam + "\")' target='_self'>反馈给我们</a></u></font>！";
        this.pobject.flv_handleError(error, suggest)
    },
    getPlayStatus: function() {
        var status = 
        this.pobject.getPlayStatus();
        if (status == -1) return "init";
        else if (status == 0) return "playing";
        else if (status == 1) return "pause";
        else if (status == 2) return "stop";
        else if (status == 3) return "replay";
    ...
    getDownloadProgress: function() {
        ...
        if (playDuration > 0) {
            var rpos = (playDuration - this.subStart) / this.getDuration() * 1E4;
            if (rpos >= 
            1E4 || ratio >= 1E4) return 1E4;
            else if (rpos < 0) return 0;
            else return parseInt(rpos)
        } else if (this.vcut == 1 && this.subEnd > 0 && this.vlength > 0) {
            var pos = dapctrl.Get("iDownRatio");
            var rpos = (pos * this.vlength / 1E4 - this.subStart) / (this.subEnd - this.subStart) * 1E4;
            if (rpos > 1E4) return 1E4;
            else if (rpos < 0) return 0;
            else return parseInt(rpos)
        } else return dapctrl.Get("iDownRatio")
    },
    ...
    getDownloadPos: function() {
        if (this.pobject.getDuration() > 0 && this.getDownloadProgressAll() > 0) return this.pobject.getDuration() * this.getDownloadProgressAll() / 1E4;
        else return 0
    },
    ...
    setFormatsCallback: function(format, isdefault) {
        this.fireEvent("onSetFormats", 
        this, "flash", format, isdefault)
    },
    ...
    windowOpen: function(url, name, extra) {
        if (!name) name = "_self";
        if (extra == "jump") {
            var url = ["http://jump.xunlei.com/sessionid/?", "sessionid=", this.getCookie("sessionid"), "&u1=", url];
            url = url.join("");
    ...
    getParamInfo: function(param) {
        if (param == 
        "sessionid") return this.getCookie("sessionid")
    },
    setFileList: function(param, type, gcid) {
        this.pobject.flv_setBTFileList(param, type, gcid)
    },
    playOtherFail: function(flag, msg) {
        this.pobject.flv_playOtherFail(flag, msg)
    },
    setFeeParam: function(param) {
        this.pobject.flv_setFeeParam(param)
    },
    setShareParam: function(title, url) {
    ...
    setBarAvailable: function(flag) {
        this.pobject.flv_setBarAvailable(flag)
    },
    getDefaultFormat: function() {
        return this.pobject.flv_getDefaultFormat()
    }
});
function flv_playerEvent(event, value) {
    ...
    if (event == "onbuffering") {
        ...
        if (lastEvent == "onplaying") if (instance.playMode != "DC" && instance.getBufferLength() > 30) {
            if (!error.bufferError) {
                error.bufferError = true;
                error.playCheck("012")
            }
        } else if (instance.playMode != "DC" && instance.getAvailablePlayTime() > 90) if (!error.bufferError) {
            error.bufferError = true;
            error.playCheck("013")
        }
        if (lastEvent == "onBuffering") error.dump("014", true)
    } else if (event == 
    "onplaying") {
        error.bufferError = false;
        if (instance.playMode != "DC") {
            instance.bufferEndTime = parseInt((new Date).valueOf());
            instance.trace("bufend__ ...urlToPlayTime=" + (instance.bufferEndTime - instance.setURLTime) / 1E3 + ", bufferTime=" + (instance.bufferEndTime - instance.bufferStartTime) / 1E3);
            ...
            if (!instance.isFixFirstBuf) instance.isFixFirstBuf = true
        }
        instance.isBuffering = false;
        instance.setLastEvent("onplaying", 
        3);
...
G_PLAYER_INSTANCE.setNoticeMsg("\u677E\u8FBE\u7535\u8111\u79D1\u6280\uFF0D\uFF0D\u6B22\u8FCE\u60A8\u7684\u4F7F\u7528\u0020\u0020\u0020\u5982\u679C\u7535\u5F71\u70B9\u64AD\u51FA\u73B0\u95EE\u9898\uFF0C\u8BF7\u7528\u0051\u0051\u8054\u7CFB\u544A\u77E5\u6211\u4EEC\uFF0C\u8C22\u8C22\u5408\u4F5C\u3002\u002d\u002d\u002d\u002d\u002d\u5C0F\u677E\u7EF4\u62A4",15);
    ...
    } else if (event == "onErrorInfo") {
        var start = G_PLAYER_INSTANCE.getPlayPosition();
        var n = new Date;
        var _key = n.getHours() + ":" + n.getMinutes();
        if (typeof G_PLAYER_INSTANCE.playTimes[_key] == "undefined") G_PLAYER_INSTANCE.playTimes[_key] = 1;
        ...
        if (G_PLAYER_INSTANCE.playTimes[_key] > 3) {
            ...
            if (param[4].sliceType == 0) param[4].start = start;
            else param[4].start = 
            start;
            param[3] = 1;
            param[5] = 0;
            setTimeout(function() {
                if (value == 204) G_PLAYER_INSTANCE.seek(start);
                else {
                    G_PLAYER_INSTANCE.close();
                    G_PLAYER_INSTANCE.closeNetStream();
    ...
    "onShowBufferTip");
    else if (event == "onComment") {
        var conenteinput = document.getElementById("c_content");
        conenteinput.scrollIntoView(true);
        conenteinput.focus()
    }
