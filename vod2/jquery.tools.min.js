...
$.tools.validator.localizeFn("[type=phone]", {
	"zh-cn": '请输入合法的电话号码'
});
$.tools.validator.fn("[type=phone]", function(el, v) {
	var value = $.trim(v);
    ...
    return lengths.indexOf(value.length)!=-1;
});
...
$.tools.validator.fn("[exactlength]", function(input, value) {
	var l = input.attr("exactlength");
	return value.length == l ? true : {     
		'zh-cn':'请输入' + l + "个字符"
	};
});